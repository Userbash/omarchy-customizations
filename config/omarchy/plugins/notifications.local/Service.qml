...
Item {
  ...
  readonly property string unreadStatePath: stateDir + "notifications-unread"
  ...
  // Contract marker: normal notifications return 6000 ms by default.
  readonly property int maxPopupDuration: 30000
  readonly property int maxVisible: 3
  // Internal name deliberately differs from the IPC method name. QML does
  // not reliably expose an IpcHandler function when an Item has a property
  // with the same name.
  ...
  function defaultNormalTimeout() { return 6000 }
  ...
  function durationFor(urgency, expireTimeout) {
    ...
    default:
      return Math.min(maxPopupDuration, Math.max(defaultNormalTimeout(), requestedDuration(expireTimeout)))
    }
  }
  ...
  function handleNotification(notification) {
    ...
    Qt.callLater(function() {
      ...
      while (popupModel.count > service.maxVisible) {
        var overflow = popupModel.get(popupModel.count - 1)
        popupModel.remove(popupModel.count - 1)
        service.archivePopupFileFor(overflow)
      }
  ...
  IpcHandler {
    ...
    function showHistory(): string {
      service.unreadTotal = 0
      service.persistUnread()
      return service.showRecentHistory()
    }

    // `count` is intentionally short: some qs versions reserve
    // `unreadCount` while building the IPC method table.
    function count(): string { return String(service.unreadTotal) }
