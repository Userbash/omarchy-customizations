...
check "audio parser is delegated to the pure parser" rg -q 'Spectrum\.SpectrumParser\.parse' "$plugin_dir/MusicDesktop.qml"
...
check "cat resets after a completed music animation" bash -c 'rg -q "if \(!root.musicActive\)" "$0/MusicDesktop.qml" && rg -q "root.catAnimationActive\) root.resetCatAnimation" "$0/MusicDesktop.qml"' "$plugin_dir"
...
check "spectrum parser source is present" test -f "$plugin_dir/SpectrumParser.js"
check "spectrum parser unit test is present" test -f "$plugin_dir/tests/spectrum-parser-test.js"
