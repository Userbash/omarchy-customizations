...
Panel {
  ...
  readonly property var effectiveLocationState: configuredLocationState.latitude !== null && configuredLocationState.longitude !== null
    ? configuredLocationState : autoLocationState
  readonly property string locationQuery: Model.wttrLocationQuery(effectiveLocationState.name, effectiveLocationState.latitude, effectiveLocationState.longitude)
  ...
  Process {
    id: locationProc
    // Resolve the city through the physical uplink when a full-tunnel VPN is
    // active. Weather requests themselves continue to use the normal route.
    command: [Quickshell.env("HOME") + "/.local/share/omarchy/weather/weather-backend", "--refresh", "--json"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        try {
          ...
          if (!isNaN(lat) && !isNaN(lon)) {
            root.autoLocationState = {
              name: String(value.locationName || ""), latitude: lat, longitude: lon
            }
            root.wttrLocation = String(value.locationName || "")
          }
        } catch (error) {
          console.warn("weather: auto-location backend response rejected: " + error)
        }
      }
    }
  }
